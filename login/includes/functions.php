...
<?php
      ...
      function login($email, $password, $mysqli) {
          ...
          if ($stmt = $mysqli->prepare("SELECT id, terminal, username, password, sign
              FROM datos
             WHERE email = ?
              LIMIT 1")) {
              ...
              $stmt->bind_result($user_id, $terminal, $username, $db_password, $sign);
              $stmt->fetch();


              // Hace el hash de la contraseña con una sal única.
              $password = hash('sha512', $password . $sign);
              if ($stmt->num_rows == 1) {
                  ...
                  if (checkbrute($user_id, $mysqli) == true) {
                      ...
                      if ($db_password == $password) {
                          ...
                          $_SESSION['user_id'] = $user_id;
                            ...
                            // Protección XSS ya que podríamos imprimir este valor.
                          $username = preg_replace("/[^a-zA-Z0-9_\-\s]+/",
                                                                      "",
                                                                      $username);
